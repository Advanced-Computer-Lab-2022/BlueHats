...
const gradeExam = async (req, res) => {
  ...
  if (flag == false) {
    const updatedTrainee = await CorporateTrainee.findOneAndUpdate(
      { _id: idTrainee },
      { grade: ans }
    );
...
const setAnswer = async (req, res) => {
  ...
  const finalT = await CorporateTrainee.findOneAndUpdate(
    { _id: id },
    { answers: ans }
  );
  console.log(finalT);
...
const getCertificateCoTrainee = async (req, res) => {
  ...
  try {
    ...
    const message = `
        ...
        <p>Thank for Completing your course</p>
        <p>Regards...</p>
    `;
